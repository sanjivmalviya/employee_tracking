<?php
 ...
 if(isset($_POST['submit'])){
      ...
      if($_POST['target_month']==""){
          ...
          $annuall_rankings = "SELECT SUM(target_category_amount) as target_amount,sales_person_id,target_year FROM tbl_target WHERE added_by = '$login_id' AND target_year = '$target_year' GROUP BY target_year ORDER BY target_id DESC";
         ...
         if(isset($annuall_rankings)){
            ...
            foreach($annuall_rankings as $rs){
               ...
               $total_orders = "SELECT COALESCE(COUNT(*),0) as total_order FROM tbl_orders WHERE sales_person_id = '$sales_person_id' AND YEAR(updated_at) = $target_year";
               $total_orders = getRaw($total_orders);
               $total_orders = $total_orders[0]['total_order'];

               $total_order_amount = "SELECT COALESCE(SUM(sales_person_order_amount),0) as total_order_amount FROM tbl_sales_person_target_detail WHERE sales_person_id = '$sales_person_id' AND YEAR(created_at) = $target_year ";
          ...
          $annuall_rankings = "SELECT SUM(target_category_amount) as target_amount,sales_person_id,target_year FROM tbl_target WHERE added_by = '$login_id' AND target_year = '$target_year' AND  target_month = '$target_month' GROUP BY target_year ORDER BY target_id DESC";
          $annuall_rankings = getRaw($annuall_rankings);
   ...
   $annuall_rankings = "SELECT SUM(target_category_amount) as target_amount,sales_person_id,target_year FROM tbl_target WHERE added_by = '$login_id' GROUP BY target_year ORDER BY target_id DESC";
   $annuall_rankings = getRaw($annuall_rankings);
    ...
    if(isset($annuall_rankings)){
      ...
      foreach($annuall_rankings as $rs){
            ...
            $year = $rs['target_year'];
            $sales_person_id = $rs['sales_person_id'];
            $total_orders = "SELECT COALESCE(COUNT(*),0) as total_order FROM tbl_orders WHERE sales_person_id = '$sales_person_id' AND YEAR(updated_at) = $year ";
            $total_orders = getRaw($total_orders);
            $total_orders = $total_orders[0]['total_order'];

            $total_order_amount = "SELECT COALESCE(SUM(sales_person_order_amount),0) as total_order_amount FROM tbl_sales_person_target_detail WHERE sales_person_id = '$sales_person_id' AND YEAR(created_at) = $year ";
            $total_order_amount = getRaw($total_order_amount);
            $total_order_amount = $total_order_amount[0]['total_order_amount'];
                                       ...
                                       <?php if(isset($data) && count($data) > 0){ ?>
                                          ...
                                          <tr>
                                             ...
                                             <td><?php 
                                                ...
                                                echo $sales_person_name =  $sales_person_name['sales_person_name']; 
                                             ?></td>
