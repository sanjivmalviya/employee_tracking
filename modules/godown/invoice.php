...
if(!isset($order_id) || !isExists('tbl_invoices','order_id',$order_id)){
   header('location:orders.php');
} 

// $invoices = getWhere('tbl_invoices','order_id',$order_id);
$invoices = getRaw("SELECT * FROM tbl_invoices WHERE order_id = '$order_id'");

if(isset($_GET['invoice_id'])){

    $invoice_id = $_GET['invoice_id'];

    $deleteInvoice = delete('tbl_invoices','invoice_id',$invoice_id);
    $invoiceDetails = getWhere('tbl_invoice_detail','invoice_id',$invoice_id);

    if(isset($invoiceDetails) && count($invoiceDetails) > 0){

      foreach ($invoiceDetails as $rs) {
          
        $dispatch_quantity = $rs['dispatch_quantity'];

        $order_product_id = getOne('tbl_order_detail','order_detail_id',$rs['order_detail_id']);
       
        $order_product_id = $order_product_id['order_product_id'];
        $removeStock = "UPDATE tbl_product SET product_stock = product_stock + $dispatch_quantity WHERE product_id = $order_product_id ";
        query($removeStock);
        $deleteInvoiceDetail = delete('tbl_invoice_detail','invoice_id',$invoice_id);
              ...
              <tbody>
                ...
                <?php if(isset($invoices)){ ?>
                   ...
                   <!--      <td><?php echo $rs['invoice_delivery_note']; ?></td>
                        ...
                        <td class="text-right"><a href="invoice_detail.php?invoice_id=<?php echo $rs['invoice_id']; ?>" class="btn btn-primary btn-xs"><i style="font-size: 15px;" class="fa fa-print"></i> Print Invoice</a> 
                          <!-- <a onclick="return confirm('Are you sure ?');" href="invoice.php?order_id=<?php echo $rs['order_id']; ?>&invoice_id=<?php echo $rs['invoice_id']; ?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a> -->
                        </td>
                      </tr>
