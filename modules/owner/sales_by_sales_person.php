<?php
 ...
 $categories = getAll('tbl_category');
   ...
   if(isset($_POST['submit'])){
     ...
     $target_assigned = "SELECT target_category_id,COALESCE(SUM(target_category_amount),0) as target_assigned,sales_person_id FROM tbl_target WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' AND target_category_id = '$category_id' AND sales_person_id = '$sales_person_id' GROUP BY target_category_id,sales_person_id";
     ...
     // all by default
     $target_assigned = "SELECT target_category_id,COALESCE(SUM(target_category_amount),0) as target_assigned,sales_person_id FROM tbl_target GROUP BY target_category_id,sales_person_id";
     $target_assigned = getRaw($target_assigned);
                     ...
                     <div class="col-sm-12">
                        <div class="card-box">
                                    ...
                                    <div class="form-group">
                                          ...
                                          <select name="sales_person_id" class="form-control select2">
                                              <?php if(isset($sales_persons) && count($sales_persons) > 0){ ?>
                                                  ...
                                                  <?php foreach($sales_persons as $rs){ ?>

                                                      <option value="<?php echo $rs['sales_person_id']; ?>"><?php echo $rs['sales_person_name']; ?></option>
                           ...
                           <div class="row" style="margin-top: 10px;">
                              ...
                              <?php if(isset($year) && isset($month) && isset($sales_person_id) && isset($category_id)){ 
                              ...
                              ?>
                                  <h4 style="padding-right: 20px;"><i> <?php echo $sales_person_name."'s Sales in ".$month."-".$year." for Category ".$category_name; ?></b></i></h4>
                                 <?php } ?>
                                 <br> 
